...
fn fft_spectrum_evolve(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let align = max(dot(normalize(k), wind), 0.0); let peak = GRAVITY / max(speed * speed, 1.0); let spectrum = exp(-peak * peak / max(kl * kl, 1e-5)) / max(kl * kl * kl * kl, 1e-5) * mix(0.08, 1.0, align * align) * sea * water.wave_amp;
